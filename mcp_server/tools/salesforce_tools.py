...
def register_salesforce_tools(mcp):
    ...
    def create_salesforce_lead(
        ...
        logger.info(f"📊 Salesforce Lead 생성 요청: {customer_name} ({customer_company}) (user: {current_user})")
        ...
        try:
            ...
            lead_id = salesforce_service.create_lead(customer_info, user_id=current_user)
            
            if lead_id:
                lead_url = salesforce_service.get_lead_url(lead_id, user_id=current_user)
    ...
    def verify_salesforce_lead(lead_id: str) -> Dict:
        ...
        """
        current_user = get_current_user()
        logger.info(f"🔍 Salesforce Lead 확인 요청: {lead_id} (user: {current_user})")

        try:
            lead_info = salesforce_service.verify_lead(lead_id, user_id=current_user)
    ...
    def get_salesforce_status() -> Dict:
        ...
        """
        current_user = get_current_user()
        logger.info(f"📊 Salesforce 상태 확인 요청 (user: {current_user})")

        try:
            if current_user:
                status = salesforce_service.get_user_service_status(current_user)
            else:
